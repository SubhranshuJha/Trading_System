...
const TradePanel = ({ stock }) => {
  ...
  const estimated = quantity > 0 ? quantity * currentPrice : 0;
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      setStatusMessage('');
      const endpoint =
        tradeSide === 'BUY' ? '/api/order/buy' : '/api/order/sell';
      await api.post(endpoint, {
        stockSymbol: stock.symbol,
        quantity: Number(quantity),
        price: Number(currentPrice),
        category: orderType === 'Market' ? 'MARKET' : 'LIMIT',
      });
      setStatusMessage(`${tradeSide} order placed successfully`);
      setQuantity(0);
    } catch (requestError) {
      setStatusMessage(
  ...
  return (
    <div className="sticky top-20 rounded-2xl border border-slate-800 bg-[#081225] p-5">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-semibold">Trade {stock.symbol}</h2>
        ...
        <div className="flex items-center gap-2 rounded-md border border-green-500/20 bg-green-500/10 px-2 py-1">
          ...
          <span className="text-[11px] font-medium text-green-400">OPEN</span>
        </div>
      </div>
      ...
      <div className="mt-5 rounded-xl border border-slate-800 bg-slate-950/70 p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[11px] uppercase text-slate-500">Current Price</p>
            <h3 className="mt-1 text-2xl font-bold">₹{stock.price}</h3>
          </div>
      ...
      {statusMessage ? (
        <p className="mt-2 text-center text-xs text-cyan-300">{statusMessage}</p>
      ) : null}
    </div>
  );
};
